...
쌓아 나가는 아니다. 레드햇은 대체 어디부터 단순 "리빌더"라고 말하는 것일까?
...
프로젝트가 문제가 있는 것처럼 통해 비판을 덧붙인다.

이렇게 말하는 게 정당할까? 꽤 알려진 파생 배포판 중에서 정말 단순 리빌드만
하는 파생 배포판 같은 것은 없다. 당연히 RockyLinux/AlmaLinux도 버그를 수정하고
업스트림에 기여한다. 물론 레드햇은 다른 어떤 회사와 비교해도 오픈소스 기여가
월등히 많은 편이므로 자신있게 말할 자격이 있지만, 다운스트림을 상대로 이렇게
말할 수는 없다고 생각한다. 레드햇 역시 다운스트림으로서 수많은 업스트림
오픈소스 때문에 기업으로서 많은 이익을 보고 있지만 그렇다고 레드햇이 그 수백
수천의 업스트림의 몫을 챙겨 줬다고 할 수 있을까? 오픈소스 생태계가 동작하는
방식이고 시장이 동작하는 방식이라고 정당화할 수 있겠지만, 그렇게 말할 수
있으려면 파생 배포판들도 생태계의 일원일 뿐이라고 말할 수 있어야 한다.
